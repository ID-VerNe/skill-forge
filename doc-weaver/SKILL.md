...
- **格式继承自 lat.md，但不依赖 lat.md 工具链**：用 Claude Code agent 实现同等能力
- **Verification 是核心信任机制**：AI 生成的文档只有经过源码级验证才可信
- **层级分离**：Tier 1（入口）+ Tier 2（知识图谱）+ Tier 3（结构化数据）三层解耦
- **模块并行**：Phase 2 各模块文档可并行生成；Phase 5 验证也可并行执行
- **自文档化**：lat.md/ 本身遵循自己定义的规范，`lat.md/lat.md` 索引所有文件